...
@SecuritySchemes(value = {
		...
		@SecurityScheme(securitySchemeName = "oauth2", type = SecuritySchemeType.OAUTH2,
				flows = @OAuthFlows(authorizationCode = 
					@OAuthFlow(
							authorizationUrl = "http://www.example.org/authorize", 
							tokenUrl = "http://www.example.org/token",
							scopes = @OAuthScope(name = "user", description = "User Scope"))))})
@PermitAll
@Path("/openapi-ui/")
public class OpenApiConfiguration implements OASFilter {

	/**
	 * Override the authorization and token URLs from configuration.
	 */
	@Override
	public org.eclipse.microprofile.openapi.models.security.SecurityScheme filterSecurityScheme(
		...
		if (org.eclipse.microprofile.openapi.models.security.SecurityScheme.Type.OAUTH2.equals(securityScheme.getType())) {
			org.eclipse.microprofile.openapi.models.security.OAuthFlow code = securityScheme.getFlows().getAuthorizationCode();
			if (code != null) {
				// QUESTIONABLE Do I really need to specify class loader?
				Config config = ConfigProvider.getConfig(OpenApiConfiguration.class.getClassLoader());

				String setAuthorizationUrl = config.getOptionalValue("oauth2.authorization.url", String.class).orElse(code.getAuthorizationUrl());
				code.setAuthorizationUrl(setAuthorizationUrl);

				String tokenUrl = config.getOptionalValue("oauth2.token.url", String.class).orElse(code.getTokenUrl());
				code.setTokenUrl(tokenUrl);
			}
		}
